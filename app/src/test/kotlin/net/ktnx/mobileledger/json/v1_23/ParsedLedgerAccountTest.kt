...
/**
 * Unit tests for [ParsedLedgerAccount] in v1_23 API.
 */
class ParsedLedgerAccountTest {
    ...
    @Test
    fun `getSimpleBalance preserves amount style`() {
        val account = ParsedLedgerAccount().apply {
            aibalance = listOf(
                ParsedBalance().apply {
                    acommodity = "EUR"
        ...
        assertEquals(1, balances.size)
        assertEquals("EUR", balances[0].commodity)
        assertEquals(123.45f, balances[0].amount, 0.01f)
    }
}
