...
/**
 * Unit tests for [ParsedLedgerTransaction] in v1_50 API.
 */
class ParsedLedgerTransactionTest {
    ...
    @Test
    fun `tindex setter propagates to postings as string`() {
    ...
    @Test
    fun `addPosting sets transaction index as string on posting`() {
        val parsed = ParsedLedgerTransaction().apply {
            tpostings = mutableListOf()
            tindex = 55
        }
    ...
    @Test
    fun `init creates default sourcepos`() {
        val parsed = ParsedLedgerTransaction()

        assertEquals(2, parsed.tsourcepos.size)
        assertEquals(2, parsed.tsourcepos[1].sourceLine)
    }
    ...
    fun `default tstatus is Unmarked`() {
        ...
        assertEquals("Unmarked", parsed.tstatus)
    }

    @Test
    fun `default tprecedingcomment is empty`() {
        val parsed = ParsedLedgerTransaction()

        assertEquals("", parsed.tprecedingcomment)
    }
}
