...
/**
 * Unit tests for [ParsedLedgerTransaction] in v1_19_1 API.
 *
 * Tests verify:
 * - toDomain() conversion
 * - fromDomain() conversion
 * - tindex propagation to postings
 * - addPosting behavior
 */
class ParsedLedgerTransactionTest {
    ...
    @Test
    fun `toDomain preserves non-empty comment`() {
        val parsed = ParsedLedgerTransaction().apply {
            tdate = "2026-01-21"
            tdescription = "Test"
            tcomment = "  Important note  "
            tpostings = mutableListOf()
        }

        val domain = parsed.toDomain()

        assertEquals("Important note", domain.comment)
    }
    ...
    fun `toDomain converts postings`() {
        ...
        val parsed = ParsedLedgerTransaction().apply {
            tdate = "2026-01-21"
            tdescription = "Grocery"
            tpostings = mutableListOf(posting)
        }

        val domain = parsed.toDomain()

        assertEquals(1, domain.lines.size)
        assertEquals("Expenses:Food", domain.lines[0].accountName)
    }
    ...
    @Test
    fun `addPosting handles null postings list`() {
        val parsed = ParsedLedgerTransaction().apply {
            tpostings = null
            tindex = 55
        }
        ...
        val posting = ParsedPosting()
        // Should not throw - just sets ptransaction_ on posting
        parsed.addPosting(posting)
