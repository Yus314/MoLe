 ...
 * ```
 */
suspend inline fun <T> safeCall(mapper: AppExceptionMapper, crossinline block: suspend () -> T): Result<T> = try {
 ...
 * ```
 */
fun <T> Flow<T>.asResultFlow(mapper: AppExceptionMapper): Flow<Result<T>> =
 ...
 *
 * 既にAppExceptionでない場合、AppExceptionMapperを使用して変換する。
 */
fun <T> Result<T>.mapErrorToAppError(mapper: AppExceptionMapper): Result<T> = this.fold(
 ...
 * ```
 */
fun Throwable.toAppError(mapper: AppExceptionMapper): AppError = when (this) {
